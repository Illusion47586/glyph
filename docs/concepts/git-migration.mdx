...
The result still exports cleanly to GitHub. For this project, the public mirror is [Illusion47586/glyph](https://github.com/Illusion47586/glyph).
...
Git asks you to choose history shape while you are still discovering the work. That is tolerable for humans and clumsy for agents. Agents often need to try edits, checkpoint partial progress, coordinate with other agents, and only later decide what should become visible. Glyph moves that decision to publication time.
...
For complete command sequences, use [Workflow Reference](/docs/cli/workflow-reference).
