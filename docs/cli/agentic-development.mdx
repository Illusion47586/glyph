...
Git has commits and branches, but it does not know which intermediate states are scratch work, which states are meaningful checkpoints, which actor currently owns the task, or which source view is safe to publish.
...
For command-by-command flows, use [Workflow Reference](/docs/cli/workflow-reference). The short policy is:
