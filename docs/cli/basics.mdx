...
For Git users, the main adjustment is that Glyph commands describe source-control intent directly. A write records a reason. A checkpoint records a milestone. A publication changes a realm. A remote sync exports that realm to GitHub.
...
See [Workflow Reference](/docs/cli/workflow-reference) for complete flows and [Command Reference](/docs/cli/command-reference) for every command, flag, argument, and prototype status.
